...
**"OPTIONAL"** in this document are intentionally non-normative; interpret them
as you choose.
...
3. A normal version number SHOULD take the form `X` where `X` is a positive
   integer. `X` is the version. This element MAY increase numerically. For
   instance: `1` -> `2` -> `3`. Alternatively: `1` -> `4` -> `5`

4. Once a versioned package has been released, the contents of that version
   SHOULD NOT be modified. Any modifications SHOULD be released as a new
   version.
...
Incrementing by 1 is RECOMMENDED; skipping numbers is allowed.
...
Then don't, nobody cares. Well I mean someone might, but they're get left behind
if they get bothered by something as trivial as API documentation!
...
Nope. You can have a 12-billion-character version for all I care. If you find
yourself running out of numbers, then consider that your software may be too
good.

### Is `v1` a valid zen version?

No, `v1` is not valid. ZenVer allows either a plain number (e.g., `1`) or an
optional `z` prefix (e.g., `z1`); `v` prefixes are not allowed. In other words,
`z1` and `1` both conform to the ZenVer spec.
...
Zen Versioning ("ZenVer") is SemVer if it had _only_ the MAJOR component and if
everything else was omitted.

A ZenVer release from 3 -> 4 is just a SemVer release from 3.0.0 to 4.0.0.
...
made too much sense to discard. This has led to the release of specification
**1**. ZenVer draws inspiration from [SemVer](https://semver.org) and
...
[^1]: This is software. Strictly speaking, it _does not have the ability to
    suck_.

[^2]: You might have seen the recent changes to `keepassxc` in Debian Sid
...
[^3]: Which there are a lot of. Although it would be reasonable to address each
    one of those limitations, I will not be doing that. Should be obvious by now
    that this is not a serious document.
